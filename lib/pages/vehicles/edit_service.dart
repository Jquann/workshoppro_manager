...
class _EditServiceState extends State<EditService> {
  ...
  TextEditingController(text: _fmt(widget.record.date));
  late final TextEditingController _desc =
  TextEditingController(text: widget.record.description);
  late final TextEditingController _mech =
  TextEditingController(text: widget.record.mechanic);
  ...
  final Map<String, InventoryPartVM> _invIndex = {};
  final Map<String, int> _stockDeltas = {};
  ...
  void _onAddPartFromInventory() {
    ...
    if (q <= 0 || q > _selectedPart!.quantity) return;
    setState(() {
      _parts.add(PartLine(
  ...
  Widget _laborEditor() => Row(children: [
    Expanded(
        child: TextField(
            controller: _laborName, decoration: _input('Name'))),
    const SizedBox(width: 8),
    SizedBox(
        width: 80,
        child: TextField(
    ...
    IconButton(
      ...
      onPressed: () {
        ...
        setState(() {
          _labor.add(LaborLine(
              name: _laborName.text.trim(), hours: h, rate: r));
          _laborName.clear();
          _laborHours.clear();
          _laborRate.clear();
        });
  ...
  Widget _pill(String text, {required VoidCallback onDelete}) =>
      Container(
  ...
  Widget _partPillWithStock(PartLine p) {
    ...
    return _pill('$top', onDelete: () {
      setState(() => _parts.remove(p));
    });
  }
  ...
  Future<void> _onSave() async {
    ...
    for (final e in _stockDeltas.entries) {
      ...
      await FirestoreService()
          .reduceStock(parts[0], parts[1], e.value);
    }
